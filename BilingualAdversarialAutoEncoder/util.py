...
class Utils:
    ...
    def run(self):
        print("Reading source word embeddings from {} ...".format(self.src_file))
        word2vec_src = self.save_word_vectors(self.src_file, save=True, save_file_as='src')
        print("Done.")
        print(word2vec_src.shape)
        print("Reading target word embeddings from {} ...".format(self.tgt_file))
        ...
        print("Constructing source word-id map from {}...")
    ...
    def save_word_vectors(self, file, save=False, save_file_as='src'):
        ...
        keys = []
        count = 0
        with io.open(self.data_dir + file, 'r', encoding='utf-8',newline='\n',errors='ignore') as f:
...
def load_npy_two(data_dir, src_fname, tgt_fname, dict=False):
    print("load file1 from {}".format(data_dir + src_fname))
    print("load file2 from {}".format(data_dir + tgt_fname))
    if dict:
...
def map_dict2ids(data_dir, dict_fname='validation.npy'):
    ...
    for src_wrd, tgt_list in dict_wrd.items():
        if src_wrd in src_ids:
            dict_ids[src_ids[src_wrd]] = [tgt_ids[tgt_wrd] for tgt_wrd in tgt_list if tgt_wrd in tgt_ids]
    return dict_ids
