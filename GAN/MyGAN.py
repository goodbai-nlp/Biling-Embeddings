...
we1 = WordEmbeddings()
we1.load_from_word2vec(dataDir, 'en')
...
we2 = WordEmbeddings()
we2.load_from_word2vec(dataDir, 'zh')
...
for epoch in range(num_epochs):
    ...
    for g_index in range(g_steps):
        ...
        g_res_loss = 1.0 - torch.mean(criterion2(d_input_data.cuda().float(),g_fake_data))

        loss = g_error+ g_res_loss
        loss.backward()
        # g_error.backward()
        # g_res_loss.backward()
    ...
    if (epoch> 10000) and (g_error.data[0] < gloss_min):
        gloss_min = g_error.data[0]
        W = G.map.weight.data.cpu().numpy()
        torch.save(G.state_dict(), 'g_params.pkl')
        print("recon_gen_loss_val:{}  ||W^T*W - I||:{}".format(g_res_loss.data[0],
                                                               np.linalg.norm(np.dot(W.T, W) - np.identity(d))))


G2 = torch.load('g_params.pkl')
